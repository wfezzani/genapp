...
echo '###########################' | tee -a $LOG_FILE
echo ' ' | tee -a $LOG_FILE
echo "  ssh $USER_HOST WORK=$WORK_DIR APP=$APP_DIR " 2>&1 | tee -a $LOG_FILE

# Check for the existence of the .setup file
x=$(ssh $USER_HOST WORK=$WORK_DIR APP=$APP_DIR  '
   if [[ -e $WORK/$APP/.setup ]];
   then
      returncode=0
   else
...
echo "  ssh $USER_HOST HLQ=$HIGH_LVL_QUAL WORK=$WORK_DIR APP=$APP_DIR OUT=$OUT_DIR LOGFILE=$LOG_FILE_NAME BLDLIST=$BUILD_LIST BLDSCRPT=$BUILD_SCRIPT" 2>&1 | tee -a $LOG_FILE
ssh $USER_HOST HLQ=$HIGH_LVL_QUAL WORK=$WORK_DIR APP=$APP_DIR OUT=$OUT_DIR LOGFILE=$LOG_FILE_NAME BLDLIST=$BUILD_LIST BLDSCRPT=$BUILD_SCRIPT '
