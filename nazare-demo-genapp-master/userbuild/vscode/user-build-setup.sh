...
echo "ssh -p 1022 $USER_HOST WORK=$WORK_DIR APP=$APP_DIR OUT=$OUT_DIR LOGFILE=$LOG_FILE" | tee -a $LOCAL_LOG_DIR/$LOG_FILE
ssh -p 1022 $USER_HOST WORK=$WORK_DIR APP=$APP_DIR OUT=$OUT_DIR LOGFILE=$LOG_FILE '
...
then
    for line in $(cat $DIR_LIST); do
        ...
        echo "ssh -p 1022 $USER_HOST DIR=$field1 OUT=$OUT_DIR LOGFILE=$LOG_FILE" | tee -a $LOCAL_LOG_DIR/$LOG_FILE
        ssh -p 1022 $USER_HOST DIR=$field1 OUT=$OUT_DIR LOGFILE=$LOG_FILE '
        LOGFILE=$OUT/$LOGFILE
     ...
     echo " " | tee -a $LOCAL_LOG_DIR/$LOG_FILE
     echo "#######" | tee -a $LOCAL_LOG_DIR/$LOG_FILE
     echo "## $DIR_LIST is empty " | tee -a $LOCAL_LOG_DIR/$LOG_FILE
     echo "## No component directories will be created " | tee -a $LOCAL_LOG_DIR/$LOG_FILE
     echo "#######" | tee -a $LOCAL_LOG_DIR/$LOG_FILE
     echo " ";
fi
...
echo "ssh -p 1022 $ssh $USER_HOST WORK=$WORK_DIR APP=$APP_DIR" | tee -a $LOCAL_LOG_DIR/$LOG_FILE
ssh -p 1022 $USER_HOST WORK=$WORK_DIR APP=$APP_DIR '
cd "${WORK}/${APP}"
